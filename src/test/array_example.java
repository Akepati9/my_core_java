package test;

public class array_example {

	public static void main(String[] args) {
		// TODO Auto-generated method stub

		int [] num1 = {20,30,40,50};
		
		System.out.println("total number of variables in array are " + num1.length);
		System.out.println("value at 3rd position is  " + num1[2]);
		System.out.println("value at the end is " + num1[num1.length -1]);
		
		String[] name1 = {"abc", "def", "ghi"};
		System.out.println("value at first place is " + name1[0]);
		
		String name = "Clean India Green India";
				
		String[] arrWords = name.split(" ");
		
		System.out.println("Value at 3rd position is " + arrWords[2]);
		System.out.println("Total num of words are " + arrWords.length);
	}

}
